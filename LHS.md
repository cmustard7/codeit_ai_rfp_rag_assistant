...
1. **답변 신뢰성 검증**: LangChain 베이스라인에서 검색된 문서와 생성된 답변의 정확성을 검증할 방법이 없었음
2. **Retry로 인한 속도 저하**: LangGraph Base의 Adaptive Retrieval은 retry 발생 시 응답 시간이 1.5~5배 증가하는 근본적 한계
3. **비교 질문 처리**: 복잡한 비교 질의(예: "A 사업과 B 사업 비교")를 체계적으로 처리할 방법 부재
4. **성능과 정확도의 균형**: 속도를 희생하지 않으면서도 높은 검색 정확도를 달성
...
### 3.1 Phase 1: LangChain Baseline (11월 10-13일)

#### 구조
```
사용자 질의 → Embedding → Vector Search (문서별 청킹) → LLM 답변 생성
```

#### 주요 구현 내용
1. **문서별 독립 청킹**: 각 문서를 독립적으로 청킹하여 문서 혼재 문제 원천 차단
2. **메타데이터 중앙 관리**: `data_list.csv`를 통해 발주 기관, 사업명, 예산, 카테고리 등 구조화된 메타데이터 주입
3. **ChromaDB Vector Store**: 기본 Semantic Search 구현
4. **간단한 프롬프트 템플릿**: 검색된 문서 기반 답변 생성
...
#### 발견된 핵심 문제
**❗ 답변 신뢰성 검증 불가**
- 검색된 문서와 생성된 답변이 정확한지 확인할 방법이 없음
- 잘못된 답변을 걸러낼 수 없어 실무 적용 리스크 존재

**인식**: 평가 LLM을 도입하여 답변 품질을 검증하는 순회구조가 필요하다

**해결방안**: LangGraph 순회구조 + 평가 LLM 도입 → Phase 2로 전환
...
3. **5단계 Adaptive Retrieval**: 평가 점수가 낮을 경우 단계적으로 검색 전략을 변경
...
#### 성과
- ✅ **답변 신뢰성 검증 가능**: 평가 LLM을 통한 품질 관리
- ✅ **평가 점수 기반 품질 보장**: Score 0.75 이상만 최종 답변으로 제공
- ⏱️ **평균 속도**: 45.50초 (LangChain 대비 약 69% 증가)
- 💰 **평균 비용**: $0.0025

#### 발견된 핵심 문제
**❗ Retry로 인한 응답 시간 증가**
- retry 발생 시 응답 시간이 최소 1.5배 이상 증가
- retry 3단계(쿼리 개선)에서 추가 LLM 호출로 레이턴시 급증
- 최악의 경우 retry 5번 전부 실행 시 처음 검색의 5배 이상 시간 소요
- 어떤 질의가 retry를 유발할지 예측 불가능하여 응답 시간 불안정

**인식**: 재시도로 품질을 개선하는 것보다, **검색 정확도를 높여서 retry 자체를 없애는 것**이 본질적 해결책이다

**해결방안**: Hybrid Multi-Search로 첫 검색부터 정확한 문서 확보 → Phase 3으로 전환

### 3.3 Phase 3: Multi-Search Hybrid Strategy (11월 18-21일)
...
#### 성과 및 잔존 이슈
- ✅ **가장 높은 검색 정확도**: 첫 검색부터 정확한 결과
- ✅ **Retry 완전 제거**: 예측 가능한 응답 시간
- ✅ **비교 질문 완벽 처리**: 키워드별 독립 검색 후 병합
- ⏱️ **평균 속도**: 55.24초
- 💰 **평균 비용**: $0.0031
- ⚠️ **문제**: 여전히 55초는 길다

**인식**: 중요 키워드만 추출해서 Input Token을 최적화하면 속도를 높일 수 있지 않을까?

**해결방안**: Distillation으로 컨텍스트 압축 시도 → Phase 4로 전환

---

### 3.4 Phase 4: Distillation 실험 (11월 22-28일)

#### 배경
Multi-Search로 정확도는 확보했지만 평균 응답 시간 55초는 여전히 길다고 판단하였다. 검색된 문서에서 중요한 키워드만 추출하여 Input Token을 줄이면 LLM 처리 시간을 단축할 수 있을 것이라 예상했다.

#### 주요 구현 내용
1. **Distillation LLM 추가**: GPT-5-mini를 사용하여 검색된 문서에서 핵심 정보만 추출
2. **압축된 컨텍스트 생성**: 원본 문서 → Distillation → 핵심 키워드/문장
3. **Main LLM에 전달**: 압축된 컨텍스트로 최종 답변 생성
4. **압축률 계산**: compression_ratio 메트릭으로 효율성 추적
5. **Multi-Search 기반 유지**: Hybrid Search + Reranker는 그대로 유지

#### 실험 결과
- ⏱️ **평균 속도**: 50.88초 (Multi-Search 55.24초 대비 **약 8% 개선**)
- 💰 **평균 비용**: $0.0031 (**2% 증가**)
- ❌ **개선 폭 미미**: 8% 속도 개선은 통계적으로 유의미하지 않음
- ❌ **추가 LLM 호출 오버헤드**: Distillation LLM 호출로 인한 레이턴시 추가
- ❌ **비용 증가 > 속도 개선**: 비용 대비 효과 낮음

#### 분석 및 최종 결론
**왜 기대만큼 효과가 없었나?**
- Distillation LLM 호출 시간(~5초) + 컨텍스트 압축 효과(~4초) = 실제 순이득 미미
- Main LLM의 토큰 처리 속도가 충분히 빨라서 압축 효과가 크지 않음
- 추가 복잡도 대비 실질적 이득 없음

**최종 선택**: **Multi-Search (Phase 3)를 최종 전략으로 확정**
- 정확도 유지가 속도 8% 개선보다 중요
- 예측 가능한 응답 시간이 더 큰 가치
- 추가 LLM 호출 없이 간결한 파이프라인 유지

---

### 3.5 Phase 5: Performance Optimization & Monitoring (11월 24-28일)
